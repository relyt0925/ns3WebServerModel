...
main (int argc, char *argv[])
{
  ...
  if(csvFile==""){
	  for(uint32_t i=0;i<sinkApps.GetN();i++){
		  ...
		  //Vector sinkLocation= helpFunc::GetPosition(sinkApps.Get(i)->GetNode());
		  uint32_t receivedBytes=udpSink->GetTotalRx();
		  Ptr<OnOffApplication> onoff1 = DynamicCast<OnOffApplication>(sourceApps.Get(i));
		  //Vector sourceLocation= helpFunc::GetPosition(sourceApps.Get(i)->GetNode());
		  ...
		  //std::cout <<  i << ",_StrmUtil," << (((double)receivedBytes/(double)bytesSent)*100)
		  //<< ",SrcLoc," << sourceLocation << ",SinkLoc," << sinkLocation << ",";
	  ...
	  for(uint32_t i=0;i<sinkApps.GetN();i++){
		  ...
		  std::cout << "totEfficiency," << (((double)totalBytesReceived/(double)totalBytesSent)*100.0) << ",";
		  std::cout << "FlowNum," << i <<',';
		  std::cout << "Efficieny," << (((double)receivedBytes/(double)bytesSent)*100) << ",";
		  std::cout << "SrcLoc," << sourceLocation << ",";
		  std::cout << "SinkLoc," << sinkLocation << std::endl;
	  }
  }
  else{
	  ...
	  for(uint32_t i=0;i<sinkApps.GetN();i++){
		  ...
		  uint32_t bytesSent=onoff1->getTotalBytesSent();
		  //double streamUtilization=static_cast<double>(receivedBytes)/static_cast<double>(bytesSent);
		  //fileOUT <<  i << ",_StrmUtil," << (((double)receivedBytes/(double)bytesSent)*100)
		  //				  << ",SrcLoc," << sourceLocation << ",SinkLoc," << sinkLocation << ",";
		  totalBytesSent+=bytesSent;
		  totalBytesReceived+=receivedBytes;
	  }
	  for(uint32_t i=0;i<sinkApps.GetN();i++){
		  ...
		  fileOUT <<  (((double)totalBytesReceived/(double)totalBytesSent)*100.0) << ",";
		  fileOUT <<  i << ",";
		  fileOUT << (((double)receivedBytes/(double)bytesSent)*100) << ",";
		  fileOUT << sourceLocation << ",";
		  fileOUT << sinkLocation << std::endl;
	  }
  }
  Simulator::Destroy();
  return 0;
